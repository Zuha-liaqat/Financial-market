...
export default function ApprovalQueuePage() {
  ...
  function handleBatchApprove() {
    ...
    setItems((prev) =>
      prev.map((item) => (selectedIds.has(item.id) ? { ...item, status: 'PRODUCTION' } : item)),
    )
    setSelectedIds(new Set())
  }

  function handleApprove(id) {
    const updated = updateGeneratedPost(id, { status: 'PRODUCTION' })
    if (updated) {
      addNotification({
        type: 'approval',
        title: `"${updated.title}" approved`,
        description: `Your ${updated.platform} post has been approved and moved to production.`,
        platform: updated.platform,
        author: 'Alex Martinez',
      })
    }
    setItems((prev) => prev.map((i) => (i.id === id ? { ...i, status: 'PRODUCTION' } : i)))
  }
